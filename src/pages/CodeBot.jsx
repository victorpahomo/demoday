...
const systemMessage = { // Este mensaje explica cómo debe comportarse la AI
...
function CodeBot() {
  ...
  async function processMessageToCodeBot(chatMessages) {
    ...
    await fetch("https://api.openai.com/v1/chat/completions",// API endpoint
      {
        method: "POST",
        headers: {
      ...
      <MainLayout props="CodeBot">
        <div className='mt-5'>
          <MainContainer className='rounded-xl'>
            <ChatContainer className="h-screen" style={{ height: 'calc(100vh - 10rem)' }}>
              <MessageList
                scrollBehavior="smooth"
                typingIndicator={isTyping ? <TypingIndicator content="CodeBot está escribiendo" /> : null}
              >
                {messages.map((message, i) => {
                  console.log(message)
                  return <Message key={i} model={message} ><Avatar src="https://static3.abc.es/media/summum/2021/10/01/maxi_iglesias-kXKH--620x349@abc.jpeg" name="CodeBot" /></Message>
                })}
              </MessageList>
              <MessageInput placeholder="Escribe un mensaje" onSend={handleSend} attachButton={false} />
            </ChatContainer>
          </MainContainer>
        </div>
      </MainLayout>
